...
import { catchError, filter, fromEvent, map, merge, never, pipe, subject, subscribe, switchMap, takeUntil } from "fastrx";
...
export class Engine extends EventEmitter {
  ...
  constructor(public video: HTMLVideoElement, opt: { debug: boolean, autoPlay: boolean; } = { debug: false, autoPlay: false }) {
    ...
    let seeking = false;
    const addBuffer = async (segment: MediaSegment) => {
      const [bufferStart, bufferEnd] = [this.bufferStart, this.bufferEnd];
      this.log('Adding segment to buffer:', segment, `[${bufferStart},${bufferEnd}]`);
      const append = await mediaSourceProxy.appendSegment(segment);
      if (!append) {
        this.log('Segment already buffered:', segment);
        return false;
      }
      const [newBufferStart, newBufferEnd] = [this.bufferStart, this.bufferEnd];
      this.log('Segment added to buffer:', segment, `[${newBufferStart},${newBufferEnd}]`);
      if (segment.duration != newBufferEnd - bufferEnd) {
        this.log('Segment duration mismatch:', segment.duration, '!=', newBufferEnd - bufferEnd);
      }
      return append;
    };
    ...
    pipe(this.seekOB, switchMap(time => {
      ...
      return async sink => {
        try {
          ...
          loadingCurrent = true;
          const appended = await addBuffer(currentSegment);
    ...
    pipe(fromEvent(video, 'timeupdate'), filter(() => !seeking), takeUntil(this.destroyOB), subscribe(() => {
      ...
      if (nextSegment && !nextSegment.ready && !loadingCurrent) {
        segmentLoading = addBuffer(nextSegment);
      }
    }));
